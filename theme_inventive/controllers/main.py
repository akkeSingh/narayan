...
class ThemeCustomerPortal(CustomerPortal):
    ...
    def edit_image(self, **kw):
        res_user = request.env['res.users'].search(
            [('id', '=', request.env.user.id)]).sudo()
        if kw.get('action') == 'edit':
            res_user.image_256 = kw.get('data').split(',')[1].strip()
        else:
            for path in tools.config['addons_path'].split(','):
                try:
                    res_user.image_256 = base64.b64encode(open(os.path.join(path, 'theme_inventive', 'static', 'src', 'img', 'unknown.png'), 'rb') .read()).decode("utf-8")
                except Exception as e:
